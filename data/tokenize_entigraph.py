...
def _get_quality_graph(args: argparse.ArgumentParser) -> List[str]:
    if args.specified_input_dir is None:
        ...
        dir_name = args.specified_input_dir
...
def tokenize_quality_graph(args):
    if args.specified_input_dir is None:
        ...
        if args.sample_triplet_ratio is not None:
            assert not args.no_triplet
            bin_fname += f"_sub3={args.sample_triplet_ratio}"
        elif args.no_triplet:
            bin_fname += "_no3"
        ...
        quality = _get_quality_graph(args)
        bin_fname = os.path.basename(args.specified_input_dir)
    write_to_memmap_single(tokenize_list(args, quality), f"{bin_fname}.bin")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trimE", action="store_true", default=False)
