...
def _write_outputs(task: Task, savename: str, outputs: Iterable, prompts: Optional[Iterable] = None):
    i = 0
...
def eval_quality_qa(model_path: str, model_name: str, **kwargs):
    task = QuALITY("all")
    ...
    outputs = llama_forward(
        ...
        prompts=task.all_questions(add_document_context=True, add_thought_process=True, sep_after_question="\n"),
    )
