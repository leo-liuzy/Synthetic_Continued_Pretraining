...
for task_name in musique_page musique
...
if [ "$subsample_ratio" = "1.0" ]; then
    run_name="${final_task_name}-lr${lr}-rr${rr}-epochs${epochs}-bs${bs}-wd${wd}-warmup${warmup}-norm${max_grad_norm}-${lr_scheduler_type}-ngpu${gpu_count}-gradacc${grad_acc}-${pretty_name}"
else
